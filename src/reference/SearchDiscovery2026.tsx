...
export function SearchDiscovery2026(){
 ...
 // Quem digita o nome de um comércio ("Facem") também precisa achá-lo:
 // antes a busca só olhava produtos e devolvia "nenhum resultado".
 const matchedStores=useMemo(()=>{
  const term=normalize(appliedQuery.trim());
  if(!catalog||term.length<3)return[];
  return catalog.stores.filter(item=>normalize(item.name).includes(term)).slice(0,4);
 },[catalog,appliedQuery]);
 ...
 {matchedStores.length>0&&<section className="search26-storehits" aria-label="Estabelecimentos encontrados">
  <h2>Estabelecimentos encontrados</h2>
  <div className="search26-storehits__list">{matchedStores.map(item=><Link key={item.id} to={`/estabelecimento/${item.slug||item.id}`}><i style={{backgroundColor:item.color||"#14795d"}}><Store aria-hidden="true"/></i><span><strong>{item.name}</strong><small>{item.neighborhood||"Feijó-AC"} · {item.products||0} produtos</small></span><ArrowRight aria-hidden="true"/></Link>)}</div>
 </section>}
