...
function AppShellController(){const {pathname}=useLocation();useEffect(()=>{const publicApp=!pathname.startsWith('/admin')&&!pathname.startsWith('/painel-lojista')&&!pathname.startsWith('/lojista')&&!pathname.startsWith('/cadastro-lojista')&&!pathname.startsWith('/quero-vender');document.body.classList.toggle('pc-app-shell',publicApp);document.body.dataset.appRoute=pathname==='/'?'home':pathname.split('/').filter(Boolean)[0]||'home';const syncViewport=()=>document.documentElement.style.setProperty('--pc-app-height',`${window.visualViewport?.height??window.innerHeight}px`);syncViewport();window.visualViewport?.addEventListener('resize',syncViewport);window.addEventListener('orientationchange',syncViewport);return()=>{document.body.classList.remove('pc-app-shell');delete document.body.dataset.appRoute;window.visualViewport?.removeEventListener('resize',syncViewport);window.removeEventListener('orientationchange',syncViewport)}},[pathname]);return null;}
