...
export type PlatformCampaign={
  ...
  imageUrl:string|null;imagePosition:string|null;imageHasText:boolean;linkUrl:string;linkLabel:string;theme:CampaignTheme;priority:number;
...
const FESTIVAL_FALLBACK:PlatformCampaign={id:'festival-acai-2026',kind:'announcement',placement:'top_bar',title:'Festival do Açaí e Festival de Praia em Feijó',subtitle:'Pesquise e compare preços locais',imageUrl:'/banner-festival-acai-feijo-2026.png',imagePosition:null,imageHasText:false,linkUrl:'/buscar',linkLabel:'Comparar preços',theme:'indigo',priority:100,isActive:true,isDismissible:true,startsAt:'2026-08-21T00:00:00-05:00',endsAt:'2026-08-25T00:00:00-05:00',createdAt:null,updatedAt:null};

const fromRow=(row:Record<string,unknown>):PlatformCampaign=>({id:String(row.id),kind:row.kind==='advertisement'?'advertisement':'announcement',placement:'top_bar',title:String(row.title||''),subtitle:row.subtitle?String(row.subtitle):null,imageUrl:row.image_url?String(row.image_url):null,imagePosition:row.image_position?String(row.image_position):null,imageHasText:Boolean(row.image_has_text),linkUrl:String(row.link_url||'/buscar'),linkLabel:String(row.link_label||'Saiba mais'),theme:(['emerald','amber','slate'].includes(String(row.theme))?String(row.theme):'indigo') as CampaignTheme,priority:Number(row.priority)||0,isActive:Boolean(row.is_active),isDismissible:row.is_dismissible!==false,startsAt:row.starts_at?String(row.starts_at):null,endsAt:row.ends_at?String(row.ends_at):null,createdAt:row.created_at?String(row.created_at):null,updatedAt:row.updated_at?String(row.updated_at):null});
const toRow=(value:CampaignInput,userId:string)=>({kind:value.kind,placement:'top_bar',title:value.title.trim(),subtitle:value.subtitle?.trim()||null,image_url:value.imageUrl?.trim()||null,image_position:value.imagePosition?.trim()||null,image_has_text:value.imageHasText,link_url:value.linkUrl.trim()||'/buscar',link_label:value.linkLabel.trim()||'Saiba mais',theme:value.theme,priority:value.priority,is_active:value.isActive,is_dismissible:value.isDismissible,starts_at:value.startsAt||null,ends_at:value.endsAt||null,updated_at:new Date().toISOString(),updated_by:userId});
