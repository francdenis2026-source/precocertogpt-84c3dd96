...
export function parseZenoMetadata(raw: string): string | null {
  ...
  try {
    ...
    const candidates = [parsed.streamTitle, parsed.title, parsed.song, parsed.currentSong, parsed.track, parsed.metadata, parsed.now_playing];
    for (const candidate of candidates) {
      const value = clean(candidate);
      if (value) return value;
      if (candidate && typeof candidate === "object") {
        const nested = candidate as Record<string, unknown>;
        const artist = clean(nested.artist ?? nested.currentArtist), title = clean(nested.title ?? nested.song ?? nested.currentSong);
...
export function PersistentRadioProvider({ children }: { children: ReactNode }) {
  ...
  const [station, setStation] = useState(DEFAULT_RADIO), [playing, setPlaying] = useState(false), [loading, setLoading] = useState(false), [failed, setFailed] = useState(false), [volume, setVolumeState] = useState(0.78), [nowPlaying, setNowPlaying] = useState<string | null>(null);
  const streams = useMemo(() => [station.stream_url, station.fallback_url].filter(Boolean) as string[], [station]);
  ...
  useEffect(() => {
    const refresh = async () => {
      ...
      setStation((current) => {
        if (current.id === next.id && current.updated_at === next.updated_at) return current;
        wantsPlay.current = false; audioRef.current?.pause(); streamRef.current = 0; return next;
      });
    };
    void refresh(); window.addEventListener("pc:radio-config-changed", refresh); return () => window.removeEventListener("pc:radio-config-changed", refresh);
  }, []);
  useEffect(() => () => clearTimer(), []);
  useEffect(() => {
    if (typeof BroadcastChannel === "undefined") return;
    const channel = new BroadcastChannel(RADIO_COORDINATION);
    channel.onmessage = (event) => { if (event.data?.type !== "play" || event.data?.instanceId === instanceRef.current) return; wantsPlay.current = false; audioRef.current?.pause(); };
    return () => channel.close();
  }, []);
  useEffect(() => {
    if (!playing || !station.metadata_url) return;
    let source: EventSource | null = null, poll: number | undefined, closed = false;
    const update = (raw: string) => { const title = parseZenoMetadata(raw); if (title) setNowPlaying(title); };
    const fetchMetadata = async () => { try { const response = await fetch(station.metadata_url!, { headers: { Accept: "application/json" } }); if (response.ok) update(await response.text()); } catch { return; } };
    if (typeof EventSource !== "undefined") {
      source = new EventSource(station.metadata_url); source.onmessage = (event) => update(event.data); source.onerror = () => { source?.close(); source = null; if (!closed) { void fetchMetadata(); poll = window.setInterval(fetchMetadata, 30000); } };
    } else { void fetchMetadata(); poll = window.setInterval(fetchMetadata, 30000); }
    return () => { closed = true; source?.close(); if (poll) window.clearInterval(poll); };
  ...
  const tryStream = async (index: number) => {
    const audio = audioRef.current; if (!audio) return; clearTimer();
    if (index >= streams.length) { wantsPlay.current = false; setLoading(false); setPlaying(false); setFailed(true); return; }
    streamRef.current = index; audio.src = streams[index]; audio.load(); setFailed(false); setLoading(true); armTimeout();
    if (typeof BroadcastChannel !== "undefined") { const channel = new BroadcastChannel(RADIO_COORDINATION); channel.postMessage({ type: "play", instanceId: instanceRef.current }); channel.close(); }
    try { await audio.play(); } catch { if (wantsPlay.current) void tryStream(index + 1); else { clearTimer(); setLoading(false); } }
  };
