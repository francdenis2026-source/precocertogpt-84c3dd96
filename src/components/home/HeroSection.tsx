...
export function HeroSection({ products, loading, productCount, storeCount, priceCount }: { products: Product[]; loading: boolean; productCount: number; storeCount: number; priceCount: number }) {
  return <section className="pc26-hero"><div className="pc26-shell pc26-hero__grid">
    <div className="pc26-hero__content"><span className="pc26-eyebrow"><BadgeCheck aria-hidden="true" />Preços locais, decisão mais segura</span><h1>Antes de comprar, descubra <em>onde custa menos.</em></h1><p>Compare preços do comércio de Feijó em poucos segundos e escolha a loja que faz sua compra render mais.</p><LiveProductSearch id="hero-price-search" products={products} loading={loading} /><div className="pc26-hero__stats" aria-label="Cobertura do Preço Certo"><span><strong>{productCount || "—"}</strong><small>produtos</small></span><span><strong>{priceCount || "—"}</strong><small>preços registrados</small></span><span><strong>{storeCount || "—"}</strong><small>estabelecimentos</small></span></div></div>
