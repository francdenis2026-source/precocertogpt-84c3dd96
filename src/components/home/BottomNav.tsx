...
export function BottomNav() {
  ...
  return <nav className="pcx-bottom-nav" aria-label="Navegação móvel">{items.map(({ to, label, icon: Icon }) => <Link key={to} className={active(to) ? "is-active" : undefined} aria-current={active(to) ? "page" : undefined} to={to}><Icon aria-hidden="true"/><span>{label}</span></Link>)}</nav>;
}
