...
export function LiveProductSearch({ products, loading = false, compact = false, id }: LiveProductSearchProps) {
  ...
  const suggestions = useMemo(
    () => normalizedQuery.length >= 2 ? suggestProducts(products, normalizedQuery, compact ? 5 : 6) : [],
  ...
  return <div ref={rootRef} className={`pc26-live-search${compact ? " pc26-live-search--compact" : ""}`}>
    ...
    {showPanel && <div id={listId} className="pc26-live-results" role="listbox" aria-label="Sugestões de produtos">
      ...
      {loading && !products.length ? <div className="pc26-live-results__state"><LoaderCircle className="pc26-live-results__loader" aria-hidden="true" /><span>Consultando preços locais…</span></div> : suggestions.length ? <div className="pc26-live-results__list">{suggestions.map((product, index) => {
      ...
      {!loading && suggestions.length > 0 && <button className="pc26-live-results__all" type="button" onClick={searchAll}>Ver todos para “{normalizedQuery}” <ArrowRight aria-hidden="true" /></button>}
    </div>}
    <span className="sr-only" role="status" aria-live="polite">{showPanel && !loading ? `${suggestions.length} sugestões disponíveis` : ""}</span>
  </div>;
}
