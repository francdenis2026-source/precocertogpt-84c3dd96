        ...
        <div className="pcx-products" aria-busy={loading} aria-live="polite">
          {loading ? (
            Array.from({ length: 6 }, (_, index) => (
              <div className="pcx-skeleton" key={index} aria-hidden="true" />
            ))
          ) : products.length ? (
            products.map((product, index) => (
              <ProductCard featured={index === 0} key={product.id} product={product} />
            ))
          ) : (
            <div className="pcx-empty">
              <PackageSearch aria-hidden="true" />
              <strong>Os preços estão sendo atualizados.</strong>
              <p>Pesquise o catálogo completo enquanto isso.</p>
              <Link className="pcx-btn pcx-btn--primary" to="/buscar">Pesquisar produtos</Link>
            </div>
          )}
        </div>
      </div>
